...
namespace MoreDatabases
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                connection.Open();
                string inline = "SELECT [dbo].[Character].Character FROM [dbo].[Character] INNER JOIN [dbo].[Character_Map_Location] ON [dbo].[Character_Map_Location].ID = [dbo].[Character].Map_LocationID WHERE [dbo].[Character_Map_Location].Map_Location = ' '";
                using (var command = new SqlCommand(inline, connection))
                {
                    var reader = command.ExecuteReader();
